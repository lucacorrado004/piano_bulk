...
class DatabaseManager:
    ...
    def _init_db(self):
        ...
        conn.execute(
            "CREATE TABLE IF NOT EXISTS piani_bulk (piano_id TEXT PRIMARY KEY, week INTEGER, year INTEGER, bulk_date TEXT)"
        )
        conn.execute(
            """CREATE TABLE IF NOT EXISTS piano_bulk_righe (
    ...
    def salva_nuovo_piano(self, df_filtrato, week_scelta):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT OR REPLACE INTO piani_bulk VALUES (?, ?, ?, ?)", (piano_id, int(week_scelta), anno_corrente, str(date.today()))
        )
        ...
        for _, row in df_filtrato.iterrows():
            ...
            cursor.execute(
                """INSERT INTO piano_bulk_righe (piano_id, batch_number, description, site, TEMP, monitoraggio, delivery_date)
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (
                    piano_id,
                    str(row.get("BATCH NUMBER", "")),
                    str(row.get("DESCRIPTION", "")),
                    str(row.get("SITES", "")),
                    str(row.get("TEMP", "")),
                    str(row.get("MONITORAGGIO", "")),
                    d_date,
                ),
            )
        conn.commit()
    ...
    def salva_modifiche(self, piano_id, df_aggiornato):
        ...
        for _, row in df_aggiornato.iterrows():
            ...
            conn.execute(
                "UPDATE piano_bulk_righe SET flag_received=?, flag_date=? WHERE piano_id=? AND batch_number=?",
...
if "piano_attivo_id" not in st.session_state:
    st.session_state.piano_attivo_id = None
...
if st.session_state.piano_attivo_id:
    ...
    # Calcolo KPI
    tot = len(df_r)
    rec_count = df_r[df_r["flag_received"] == 1].shape[0]
